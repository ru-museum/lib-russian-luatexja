...
\end{table}\vspace{-4mm}
  ...
  詳しくは\textbf{\gtfamily 7.4 BibLaTeX内での翻字表記}を参照して下さい。
\end{itemize}
...
\begin{itemize}
  ...
  \item 連数字(\verb|\rensuji{<num>}|)を行うには、\verb|\usepackage{lltjext}|が必要です。
  \item 表記例先頭の縦向きロシア文字表記には、連数字用の\verb|\rensuji{|{\colH{800000}Л}\verb|}|を代用します。\\頭字語（МХАТなど）の単語にも用います。
\end{itemize}
\end{minipage}

\vspace{-50mm}\hspace{90mm}
...
\end{verbatim}
  ...
  \item BibLaTeX内での翻字表記については\textbf{\gtfamily 7.4 BibLaTeX内での翻字表記}を参照して下さい。
...
※ \textbf{SECTION ８} のサンプルを参照。\vspace{2mm}
